...
SOURCE_ANNOTATION_WRITER="$PROJECT_DIR/Assets/Scripts/DatasetAnnotationWriter.cs"
...
require_file "$SOURCE_ANNOTATION_WRITER"

if grep -Eq 'DatasetQAGenerator\.Generate|qa_entries\.json|qa\.txt' \
  "$SOURCE_ANNOTATION_WRITER"; then
  echo "DatasetAnnotationWriter must remain scene-state-only:" >&2
  echo "  $SOURCE_ANNOTATION_WRITER" >&2
  exit 1
fi

echo "Building a scene-state-only Player."
echo "Unity will not generate QA or load tabletop_qa_templates.json."
echo "The build only records Batch annotations, manifests, frames and videos."
...
rsync -a --delete \
  ...
  --exclude '/StreamingAssets/tabletop_qa_templates.json' \
  --exclude '/StreamingAssets/tabletop_qa_templates.json.meta' \
  "$PROJECT_DIR/Assets/" \
  "$LIGHT_PROJECT/Assets/"

rm -f \
  "$LIGHT_PROJECT/Assets/StreamingAssets/tabletop_qa_templates.json" \
  "$LIGHT_PROJECT/Assets/StreamingAssets/tabletop_qa_templates.json.meta"
...
if [[ -e "$BUILD_DIR/ChangeBlindnessRoom_Data/StreamingAssets/tabletop_qa_templates.json" ]]; then
  echo "The scene-state-only Player unexpectedly contains tabletop_qa_templates.json." >&2
  exit 1
fi
...

{
  ...
  printf 'QA mode: scene-state-only\n'
...
echo "  Runtime QA: disabled; Batch output contains scene state only"
echo "  Final JSON/QA: not generated by our runner"
echo "  Log    : $LOG_FILE"
echo
