...
- `qwen_edit_diagonal_bridge_search_residual_cache.py`：单样本、单窗口长度搜索。
- `qwen_edit_batch_window_sweep_residual_cache.py`：数据集批量窗口扫描。
- `run_qwen_window_sweep_residual_cache.sh`：双 GPU 启动、日志、状态和停止脚本。
...
chmod +x \
  qwen_edit_diagonal_bridge_search_residual_cache.py \
  qwen_edit_batch_window_sweep_residual_cache.py \
  run_qwen_window_sweep_residual_cache.sh
```
...
./run_qwen_window_sweep_residual_cache.sh start
```

查看：

```bash
./run_qwen_window_sweep_residual_cache.sh status
./run_qwen_window_sweep_residual_cache.sh log
```

停止：

```bash
./run_qwen_window_sweep_residual_cache.sh stop
```
